(function($) {
    ...
    function imageAttachment(el, options) {
        ...
        var hasCrop = options.crop || false;

        var previewWrap = $('.preview', el);
        if (hasCrop) {
            var $cropImage = $('.crop_image', el);
            $cropImage.click(function() {
                if (hasImage)
                {
                    var vData = cropImage('image/jpeg');
                    if (vData) {
                        var xhr = new XMLHttpRequest();
                        xhr.open('POST', apiUrl, true);
                        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                        var data = 'image=' + vData + '&' + 'crop=true' + (options.csrfToken ? '&' + options.csrfTokenName + '=' + options.csrfToken : '');
                        xhr.send(data);
                        updatePreview(vData);
                    }
                }
        ...
        function updatePreview(url) {
            if (url) {
                ...
                $('#GetStartForm_avatar_path').attr('value', previewUrl);
                ...
                if (hasCrop)
                {
                    ...
                    $cropImage.addClass('disabled');
                } else
                    previewImage.removeAttr('src');
                $('#GetStartForm_avatar_path').attr('value', '');
